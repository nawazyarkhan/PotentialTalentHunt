...
# build RAG context
def build_rag_context(meta, results, max_chars: int = 6000) -> str:
    """
    Build a compact context block from the retrieved candidates.
    """
    chunks = []
    used = 0
    for r in results:
        row = meta[int(r["faiss_id"])]
        txt = get_candidate_text(row)
        block = (
            f"Candidate Rank #{r['rank']} (score={r['score']}, faiss_id={r['faiss_id']}, id={row.get('id')}):\n"
            f"{txt}\n"
            f"{'-'*60}\n"
        )
...
# generate RAG answer
def generate_answer_ollama(system_prompt: str, user_prompt: str) -> str:
    """
    Fully local generation via Ollama.
    Uses /api/chat on localhost (default).
    """
    payload = {
        "model": OLLAMA_MODEL,
        "messages": [
...
#st.write("Meta keys example:", list(meta[0].keys()))